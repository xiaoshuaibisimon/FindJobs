...
int bfs(Graph *graph, BfsVertex *start, List *hops)
{
    ...
    BfsVertex          *clr_vertex,//缓存每个顶点--顶点都是BfsVertex结构体
       ...
       list_next(element))
    {
       ...
       {
          //初始化非起始顶点
          clr_vertex->color = white;//其他没访问的白色
          clr_vertex->hops = -1;//跳数-1
    ...
    {//clr_adjlist指向起始顶点（的邻接表）--顶点本身开始的地方
    ...
    {//遍历整个图的链表
       ...
       clr_vertex = (BfsVertex*)(((AdjList *)list_data(element))->vertex);//数据域是BfsVertex结构体

       if (clr_vertex->hops != -1)
       {

          if (list_ins_next(hops, list_tail(hops), clr_vertex) != 0)//将顶点指针插入hops
          {

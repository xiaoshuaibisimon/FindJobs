        ...
        int(*match)(const void * key1,const void *key2))
{
    ...
    {//遍历给出的顶点集合
        ...
        tsp_vertex = (TspVertex*)list_data(element);//取出顶点
        if(match(tsp_vertex,start))
        {//匹配上了指定的起始顶点
            if(list_ins_next(tour,list_tail(tour),tsp_vertex) != 0)
            {//1.1 直接将起始顶点插入旅行路径--作为第一个顶点
                list_destroy(tour);
                return -1;
            }

            tsp_start = tsp_vertex;//1.2 记录起始顶点
    ...
    {//O（v-1）
        //2.1 从所有白色的顶点中选择一个与tour中最后一个顶点距离最近的顶点
        minimum = DBL_MAX;
        for(element = list_head(vertices);element != NULL;element = list_next(element))
        {//2.1.1 遍历所有顶点
            tsp_vertex = (TspVertex*)list_data(element);//2.1.2 取出数据

            if(tsp_vertex->color == white)
            {//2.1.3 如果顶点是白色的

                //2.1.4 计算当前顶点与tour中最后一个顶点的距离--寻找最短距离的顶点
                distance = sqrt(pow(tsp_vertex->x - x,2.0)+pow(tsp_vertex->y - y,2.0));
                if(distance < minimum)
                {//更新最短距离和标记将要加入tour中的顶点
                    minimum = distance;
        ...
        //2.5 准备从剩余的白色顶点中选择下一个最短距离的顶点加入tour
        i++;
    }

    //3. 将起始顶点加入到tour的尾部形成闭合回路

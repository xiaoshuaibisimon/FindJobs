...
typedef struct _PathVertex
{
    ...
    struct _PathVertex * parent;
}PathVertex;

//释放边--根据当前选择的顶点，找到的邻接点匹配的链表顶点，以及当前边的权重/邻接点的权重
//权重由graph_ins_edge函数的第二个参数决定
...
{//O（EV2）
    ...
    {
        pth_vertex = (PathVertex *)(((AdjList*)list_data(element))->vertex);
    ...
    //2. 利用Dijkstra算法求出最短路径
    i = 0;
    while(i < graph_vcount(graph))
    {//每一个链表顶点都要进行单循环迭代
        minimum = DBL_MAX;

        //2.1 找出最短路径估计最小的顶点--O（v）
        for(element = list_head(&graph_adjlists(graph));element != NULL;element = list_next(element))
        {
            pth_vertex = (PathVertex *)(((AdjList*)list_data(element))->vertex);
        ...
        ((PathVertex*)adjlist->vertex)->color = black;//最短路径估计d最小的顶点对应的邻接表结构体

        //2.2 遍历邻接表--O（E）
        for(member = list_head(&adjlist->adjacent);member != NULL;member = list_next(member))
        {
            adj_vertex = (PathVertex*)list_data(member);//取出邻接顶点

            //2.3 根据邻接顶点遍历链表顶点--O（V）
            //每一个邻接顶点都要进行一次O（V）的遍历
            for(element = list_head(&graph_adjlists(graph));element != NULL;element = list_next(element))
            {
                pth_vertex = (PathVertex *)(((AdjList*)list_data(element))->vertex);
                ...
                {//找到匹配的链表顶点--释放对应的边
                 ...
                 //终止顶点是邻接顶点匹配的链表顶点
                 //使用的权重是邻接顶点的weight成员
                    relax((PathVertex*)adjlist->vertex,(PathVertex*)pth_vertex,adj_vertex->weight);//与prim算法不同的地方
                }
            }
        }
    ...
    }//寻找到了从起始顶点开始到其他所有顶点的最短路径--根据链表中成员父节点来指出精确的路径

...
typedef struct DfsVertex_ {
    ...
    VertexColor        color;//颜色

} DfsVertex;

//该函数只能访问从初始顶点adjlist开始可到达的顶点--需要针对所有顶点都调用一次--否则会有漏掉的顶点
static int dfs_main(Graph *graph, AdjList *adjlist, List *ordered)
{
    ...
    //将调用者传递的顶点设置为灰色，并遍历顶点的邻接表（将顶点的邻接顶点集合简单看成是他的邻接表）
    ((DfsVertex *)adjlist->vertex)->color = gray;//灰色代表还未发现他
    ...
    {//遍历邻接表
       ...
       //决定邻接表中顶点的颜色
       adj_vertex = (DfsVertex *)(list_data(member));

       if (graph_adjlist(graph, adj_vertex, &clr_adjlist) != 0)//在邻接表结构链表中寻找该邻接顶点位置--位置对应的是一个邻接表起始位置
          return -1;

       clr_vertex = (DfsVertex*)(clr_adjlist->vertex);//取出顶点

       //如果顶点是白色的--该邻接顶点也未被发现
       if (clr_vertex->color == white)
       {
          if (dfs_main(graph, clr_adjlist, ordered) != 0)//递归遍历该顶点的邻接表
             return -1;
    ...
    //直到所选的顶点没有其他的白色邻接顶点为止（没有剩余的邻接顶点或者是所有邻接顶点都是非白色）
    //当该顶点的邻接表中所有顶点都遍历结束（这个遍历包含了递归遍历邻接顶点的邻接顶点集合/邻接表），将该顶点设置为黑色--达到最深的程度
    ((DfsVertex *)adjlist->vertex)->color = black;//表示完成了对它的搜索--开始回归到上一个顶点继续遍历上一个顶点的邻接表

    if (list_ins_next(ordered, NULL, (DfsVertex *)adjlist->vertex) != 0)//将该顶点插入链表头
       return -1;
...
int dfs(Graph *graph, List *ordered)
{
       ...
       list_next(element))
    {
       ...
       vertex = (DfsVertex*)(((AdjList *)list_data(element))->vertex);//取出顶点
       vertex->color = white;//全部初始化为白色

    }

    //执行深度优先搜索--初始化链表--链表存储的是完成拓扑排序后返回的顶点
    list_init(ordered, NULL);

    for (element = list_head(&graph_adjlists(graph)); element != NULL; element =
       list_next(element))
    {//遍历整个链表--element对应的就是深度优先中所谓的起始顶点（实际上是包含了顶点的邻接表）

       //保证对图中所有未相连的元素完成了检索
       vertex = (DfsVertex *)(((AdjList *)list_data(element))->vertex);//取出顶点--针对每一个顶点都要执行一次该调用
